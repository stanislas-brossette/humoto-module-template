...
void setupHierarchy_v0(humoto::OptimizationProblem& opt_problem,
                       const humoto::example::ProblemParameters& params)
{
    // tasks, which are used in the control problem
    humoto::TaskSharedPointer task_cop_bounds(
        new humoto::example::TaskCoPBounds(params.gainTaskCoPBounds_));
    humoto::TaskSharedPointer task_cop_pos_ref(
        new humoto::example::TaskCoPPosRef(params.gainTaskCoPPosRef_));
    humoto::TaskSharedPointer task_com_velocity(
        new humoto::example::TaskCoMVelocity(params.gainTaskVelocity_));
                       ...
                       const humoto::example::ProblemParameters& params)
{
    ...
    humoto::TaskSharedPointer task_com_height(
        new humoto::example::TaskCoMHeight(params.gainTaskCoMHeight_));
    humoto::TaskSharedPointer task_cop_pos_ref(
        new humoto::example::TaskCoPPosRef(params.gainTaskCoPPosRef_));
    humoto::TaskSharedPointer task_com_velocity(
        new humoto::example::TaskCoMVelocity(params.gainTaskVelocity_));
    ...
    humoto::TaskSharedPointer task_kinematics_polygon(
        new humoto::example::TaskKinematicsPolygon(params.gainTaskKinematicsPolygon_));
